...
| --- | ---------------------------------- | --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| ★1  | **Linter passes**                  | All code changes                  | `pnpm lint` exits 0. No `eslint-disable` without justification.                                                                                    |
| ★2  | **Formatter passes**               | All code changes                  | `pnpm format` produces no diff. Prettier: single quotes, semicolons, 2-space indent, `arrowParens: "avoid"`, 100 char width.                       |
...
| ★4  | **Existing tests pass**            | All code changes                  | `pnpm test` (Vitest) exits 0. No `.skip` or temporarily disabled tests.                                                                            |
...
| 13  | **Build succeeds**                 | All changes (final gate)          | `pnpm build` completes. No SSR/hydration mismatches.                                                                                               |
...
| 0.5 Validate content completeness across all 3 projects                    | 1h     | 0.2, 0.3   | —         | Run `pnpm test` — all content and i18n tests pass. Each project has enough data to render every planned sub-component. **Quality gates ★1–★4.**                                                                                                                                                                                                                                                 |
...
| 5.6 Final test suite pass                                     | 2h     | 5.1–5.5    | —           | All test files pass (`pnpm test` exits 0). `pnpm build` succeeds. `pnpm lint` exits 0. `pnpm format` produces no diff. No `.skip` or `.only` in test files. **Quality gates ★1–★4, #12, #13.**                                                                                                            |
...
| 5.8 Final lint + format pass                                  | 1h     | 5.7        | —           | `pnpm lint` and `pnpm format` pass. Zero ESLint warnings. Prettier produces no diff. **Quality gates ★1–★2.**                                                                                                                                                                                             |
...
| Existing case study tests break from schema extension     | Low    | High        | Update `CaseStudy.content.test.ts` in Phase 0.2. Make `inheritanceStory` validation optional (not required for schema pass). Run `pnpm test` after every data change.       |
